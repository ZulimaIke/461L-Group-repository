  ...
  function userMessage(flag) {
    ...
    return (
        ...
        <Form.Group size="lg" controlId="username">
          ...
          <Form.Label>Enter Username to Confirm</Form.Label>
          <Form.Control
            type="user"
            value={user}
            onChange={(e) => setUser(e.target.value)}
          />
        </Form.Group>  <Button type="button" disabled={!validateEntry}
        onClick={() => {
                    fetch("/checkout/" + hwSetName + "!" + quantity + "!" + username + "!" + user)
                        .then(response => 
                            response.json()
                        )
                        .then(data => {
...
<Button type="button" disabled={!validateEntry}
        onClick={() => {
                    fetch("/checkin/" + hwSetName + "!" + quantity + "!" + username + "!" + user)
                        .then(response => 
                            response.json()
                        )
                        .then(data => {
